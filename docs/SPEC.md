...
* **Scenario B: Existing Directory Discovery (`specreg generate`)**
    ...
    5. Can generate `.spec/code-map.json` and `.spec/code-trace.json` with stable IDs, parsed code entities, code-to-spec links, coverage, drift, aliases, and embedding-profile guidance as the local sidecar foundation for traceability and reporting.
