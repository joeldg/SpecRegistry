...
2. Start the \`specregistry\` MCP server from \`.mcp.json\` and call \`get_specs\` for the project type and repo.
3. Load relevant governed procedures from \`.spec/skills/*/SKILL.md\` when present before performing that workflow.
4. Use \`search_specs\` with \`mode: "hybrid"\`, the project type, and repo when you need focused guidance from a large spec set.
5. If specs are ambiguous, contradictory, outdated, or missing intent, call \`report_spec_feedback\` with the affected \`spec_id\`, issue type, description, and relevant code or spec context.
6. Do not silently ignore a governed requirement. Either follow it or report feedback.
7. If MCP is unavailable, use only the documented agent API fallback, record that MCP was unavailable, and do not browse/probe registry routes.
8. Before reporting completion, call \`check_compliance\` or run \`specreg comply\` and keep working until the objective verdict passes.
