...
export async function stubPromptRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/cli/code-trace-report", { bodyLimit: CODE_TRACE_MAX_BYTES }, async (req) => {
    ...
    const linkCount = Array.isArray(trace.links) ? Math.min(trace.links.length, 500) : 0;
    let persisted!: { reportId: string; createdAt: string };
    const consumerId = app.db.transaction(() => {
      const id = ensureConsumer(app, body, pt.id);
      persisted = persistCodeTrace(app.db, id, trace as unknown as Record<string, unknown>);
      return id;
    })();
