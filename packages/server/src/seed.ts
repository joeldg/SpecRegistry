...
7. Agents must call \`report_spec_feedback\` or the feedback API for ambiguity, contradiction, or outdated guidance in an existing spec, and the same tool with \`error_type: "missing_guidance"\` (no \`spec_id\` required) when no existing spec covers the language/domain/topic.
...
10. Agents must reach the registry only through the MCP server, the documented agent API (\`begin_task\`, \`get_specs\`, \`search_specs\`, \`resolve_guidance\`, \`finish_task\`, \`report_token_usage\`, \`report_spec_feedback\`), and the \`specreg\` CLI. They must not browse the web dashboard, enumerate or probe other server routes, or inspect the registry's database, filesystem, or internals.
...
18. When the agent host exposes model token counts, agents should call \`report_token_usage\` with the active \`begin_task\` session id. This is observability telemetry only and does not replace the completion/compliance gate.
...
## AI Agent Directives
Use governed specs as authority. Prefer registry search over broad context loading. Stop on missing or conflicting guidance. Never treat local generated files or examples as published specifications. Stay within the MCP tools, documented agent API, and \`specreg\` CLI — do not explore or probe the registry server itself. If the compliance gate cannot run or repeated compliance attempts fail, halt and show the user the exact output.
`,
  },
  {
...
## AI Agent Directives
When work changes code structure, APIs, schemas, commands, or config, prefer generating a code trace report and mention unmapped entities. Use \`@spec[FILE#section]\` annotations only for true, specific governance links. Do not blanket-map entities to \`PROJECT_PROFILE.md\` or broad requirements just to raise coverage. Report missing spec coverage instead of pretending all code is governed.
`,
  },
  {
