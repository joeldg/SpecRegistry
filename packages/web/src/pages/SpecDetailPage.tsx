...
function AuditPromptPanel({ spec, onError }: { spec: SpecDetail; onError: (msg: string) => void }) {
  ...
  // Auto-load saved prompt on mount
  useEffect(() => {
    if (spec.id) {
      api.auditPromptGet(spec.id)
        .then((result) => {
  ...
  async function regenerate() {
    setLoading(true);
    try {
      // Use POST to generate (and persist) the prompt — supports LLM + custom guidance
      const result = await api.auditPrompt(spec.id, useLlm, useLlm ? customGuidance : undefined);
      setPrompt(result.prompt);
      setModel(null);
      setLoadError(undefined);
    } catch (e) {
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await api.updateAuditPrompt(spec.id, editedPrompt);
  ...
  return (
    <div className="section" style={{ marginTop: 28 }}>
      <h2>Audit prompt</h2>
      <div className="card">
        <div className="form-row" style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
          {!isEditing && (
            <>
              ...
              {useLlm && (
                <input
                  ...
                  placeholder="Custom focus (e.g. governance and security)"
              ...
              {prompt && (
                <button onClick={startEditing} disabled={loading} style={{ marginLeft: "auto" }}>
                  Edit
                </button>
              )}
            </>
          ...
          {isEditing && (
            <>
              ...
              <span className="dim" style={{ marginLeft: 12 }}>Editing custom prompt</span>
            </>
          )}
          {model && !isEditing && <span className="faint">Generated by {model}</span>}
        </div>
        {isEditing ? (
          <textarea
          ...
          <div className="dim">Could not load audit prompt: {loadError}</div>
        ) : (
